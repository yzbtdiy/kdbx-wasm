...
pub fn encrypt_aes256_cbc(plaintext: &[u8], key: &[u8], iv: &[u8]) -> Result<Vec<u8>, KdbxError> {
    let key: [u8; 32] = key
        .try_into()
        .map_err(|_| KdbxError::InvalidFieldValue("Key must be 32 bytes".into()))?;
    let iv: [u8; 16] = iv
        .try_into()
        .map_err(|_| KdbxError::InvalidFieldValue("IV must be 16 bytes".into()))?;
...
pub fn encrypt_chacha20(data: &[u8], key: &[u8], nonce: &[u8]) -> Result<Vec<u8>, KdbxError> {
    ...
    let nonce: [u8; 12] = nonce
        .try_into()
        .map_err(|_| KdbxError::InvalidFieldValue("Nonce must be 12 bytes".into()))?;
...
pub fn derive_key_aes_kdf(
    ...
    if password.len() != 32 {
        return Err(KdbxError::ValidationError(
            "AES-KDF input must be 32 bytes".into(),
        ));
    }
    if seed.len() != 32 {
        return Err(KdbxError::ValidationError(
    ...
    Ok(SecVec::new(transformed[..output_length].to_vec()))
}
...
pub fn compute_hmac_sha256(key: &[u8], data: &[u8]) -> Result<Vec<u8>, KdbxError> {
    let mut mac =
        HmacSha256::new_from_slice(key).map_err(|e| KdbxError::ValidationError(e.to_string()))?;
    mac.update(data);
...
mod tests {
    ...
    fn test_argon2d_derive_key() {
        let key =
            derive_key_argon2d(b"test_password", b"test_salt_123456", 65536, 3, 4, 32).unwrap();
    ...
    fn test_argon2id_derive_key() {
        let key =
            derive_key_argon2id(b"test_password", b"test_salt_123456", 65536, 3, 4, 32).unwrap();
        assert_eq!(key.len(), 32);
    }
