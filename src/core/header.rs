...
pub fn parse_header(data: &[u8]) -> Result<KdbxHeader, KdbxError> {
    ...
    loop {
        ...
        match field_id {
            ...
            id if id == FieldId::InnerRandomStreamKey as u8 => inner_random_stream_key = Some(field_data),
            id if id == FieldId::KdfParameters as u8 => kdf = Some(parse_kdf_parameters(&field_data)?),
            _ => {}
        }
    }
    ...
    Ok(KdbxHeader {
        ...
        encryption_iv: encryption_iv.ok_or_else(|| KdbxError::MissingField("encryption_iv".into()))?,
        transform_seed,
        transform_rounds,
        stream_start_bytes,
...
fn parse_kdf_parameters(data: &[u8]) -> Result<KdfAlgorithm, KdbxError> {
    ...
    loop {
        ...
        match key_str.as_ref() {
            ...
            "I" | "Iterations" => iterations = Some(u64::from_le_bytes(value[..8].try_into().unwrap())),
            "P" | "Parallelism" => parallelism = Some(u32::from_le_bytes(value[..4].try_into().unwrap())),
...
pub fn generate_header(header: &KdbxHeader) -> Result<Vec<u8>, KdbxError> {
    ...
    // Cipher ID
    write_field(&mut data, FieldId::CipherId as u8, &get_encryption_uuid(&header.encryption));

    // Compression flags
    write_field(&mut data, FieldId::CompressionFlags as u8, &header.compression.to_u32().to_le_bytes());

    // Master seed
    write_field(&mut data, FieldId::MasterSeed as u8, &header.master_seed);

    // Encryption IV
    write_field(&mut data, FieldId::EncryptionIv as u8, &header.encryption_iv);
    ...
    if let Some(rounds) = header.transform_rounds {
        write_field(&mut data, FieldId::TransformRounds as u8, &rounds.to_le_bytes());
    }
