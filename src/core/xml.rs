...
fn parse_metadata(node: Option<Node<'_, '_>>) -> Metadata {
    let Some(node) = node else { return Metadata::default() };
    Metadata {
...
fn parse_group_node(
    ...
    let mut group = Group {
        ...
        parent_id,
        icon_id: child_text(node, "IconID").and_then(|v| v.parse().ok()).unwrap_or_default(),
...
fn parse_entry_node(
    ...
    for string_node in node.children().filter(|n| n.has_tag_name("String")) {
        let Some(key) = child_text(string_node, "Key") else { continue };
...
fn parse_deleted_objects(node: Node<'_, '_>) -> Vec<DeletedObject> {
    node.children()
        ...
        .filter_map(|n| {
            let id = child_text(n, "UUID").as_deref().and_then(|v| parse_uuid(v).ok())?;
...
fn parse_value_node(
    ...
    let Some(node) = node else { return Ok(String::new()) };
...
fn parse_expires(node: Node<'_, '_>, times: Option<Node<'_, '_>>) -> Option<DateTime<Utc>> {
    let expires = child_text(node, "Expires")
        .is_some_and(|v| v.eq_ignore_ascii_case("true"));
    if expires { time_field(times, "ExpiryTime") } else { None }
}
...
fn write_group_xml(
    ...
    let mut children: Vec<&Group> = groups.values().filter(|g| g.parent_id == Some(group.id)).collect();
    children.sort_by(|a, b| a.name.cmp(&b.name).then(a.id.cmp(&b.id)));

    let mut child_entries: Vec<&Entry> = entries.values().filter(|e| e.group_id == group.id).collect();
    ...
    write_times_xml(xml, group.created_at, group.updated_at, group.updated_at, None);
...
fn write_entry_xml(
    ...
    if let Some(password) = &entry.password {
        write_string_field(xml, "Password", password.as_str(), protected_stream.as_deref_mut());
    }
    ...
    write_times_xml(xml, entry.created_at, entry.updated_at, entry.accessed_at, entry.expires_at);
    xml.push_str("</Entry>");
    Ok(())
}

fn write_string_field(xml: &mut String, key: &str, value: &str, stream: Option<&mut ProtectedStream>) {
    ...
    xml.push_str(&format!("<String><Key>{}</Key>{}</String>", escape_xml(key), value_xml));
}
